...
public enum ErrorCode {
    ...
    // Contract
    CONTRACT_NOT_FOUND(HttpStatus.NOT_FOUND, "C101", "해당 근로계약서를 찾을 수 없습니다."),
    CONTRACT_INVALID_STATUS(HttpStatus.BAD_REQUEST, "C102", "현재 상태에서는 해당 작업을 수행할 수 없습니다."),
    CONTRACT_ACCESS_DENIED(HttpStatus.FORBIDDEN, "C103", "해당 근로계약서에 접근할 권한이 없습니다."),
    CONTRACT_STORE_NOT_OWNED(HttpStatus.FORBIDDEN, "C104", "해당 매장의 근로계약서를 관리할 권한이 없습니다."),
    CONTRACT_PDF_GENERATION_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "C105", "근로계약서 PDF 생성에 실패했습니다."),
    CONTRACT_PDF_NOT_FOUND(HttpStatus.NOT_FOUND, "C106", "근로계약서 PDF 파일을 찾을 수 없습니다."),
    CONTRACT_INVALID_EMPLOYEE(HttpStatus.BAD_REQUEST, "C107", "재직 중인 직원에 대해서만 계약서를 생성할 수 있습니다."),
    ...
    DOCUMENT_FILE_EMPTY(HttpStatus.BAD_REQUEST, "D002", "업로드할 파일이 비어있습니다."),
    DOCUMENT_FILE_NOT_FOUND(HttpStatus.NOT_FOUND, "D003", "파일을 찾을 수 없습니다."),
    DOCUMENT_UPLOAD_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "D004", "파일 업로드에 실패했습니다."),
    INVALID_DOC_TYPE(HttpStatus.BAD_REQUEST, "D005", "유효하지 않은 문서 유형입니다."),
    DOCUMENT_ALREADY_SUBMITTED(HttpStatus.CONFLICT, "D006", "이미 제출된 문서입니다."),
    INVALID_EXPIRE_DATE(HttpStatus.BAD_REQUEST, "D007", "유효하지 않은 만료일 형식입니다."),
    DOCUMENT_NOT_ACTIVE(HttpStatus.BAD_REQUEST, "D008", "유효하지 않은 상태의 문서입니다."),
    INVALID_FILE_TYPE(HttpStatus.BAD_REQUEST, "D009", "허용되지 않은 파일 형식입니다."),
    DOCUMENT_FILE_TOO_LARGE(HttpStatus.BAD_REQUEST, "D010", "파일 크기가 제한을 초과했습니다."),
    DOCUMENT_INVALID_STATUS_TRANSITION(HttpStatus.BAD_REQUEST, "D011", "현재 문서 상태에서는 해당 상태로 변경할 수 없습니다.");
