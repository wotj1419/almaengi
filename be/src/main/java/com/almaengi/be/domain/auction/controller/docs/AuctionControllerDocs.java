...
public interface AuctionControllerDocs {
        ...
        @ApiResponses(value = {
                        ...
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "A003: 하한가는 법정 최저시급 이상이어야 합니다.<br>A004: 상한가는 하한가 이상이어야 합니다."),
        ...
        @Operation(summary = "경매 낙찰 확정(마감)", description = "사장님이 지원자 중 한 명 이상을 확정하고 경매를 마감합니다.")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "낙찰 성공"),
        ...
        ApiResponse<Void> closeAuction(
                        @Parameter(description = "마감할 경매 ID") @PathVariable Long auctionId,
                        @RequestBody AuctionRequestDto.Close request);

        @Operation(summary = "매장별 경매 목록 조회", description = "특정 매장에 개설된 모든 대타 경매 목록을 조회합니다.")
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = AuctionResponseDto.Auction.class)))
        ApiResponse<List<AuctionResponseDto.Auction>> getAuctions(
                        @Parameter(description = "조회할 매장 ID") @PathVariable Long storeId);

        @Operation(summary = "단일 경매 상세 조회", description = "특정 경매의 상세 정보와 지원자 정보(사장님 권한일 경우)를 조회합니다.")
        @ApiResponses(value = {
